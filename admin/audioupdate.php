<?php
include('connect.php');
// viewpage valla edit ?edit valla //
$id = $_GET['edit'] ?? "";
if (isset($_POST['update_music'])) {
	...
	$audio = $_FILES['audio'];
    $fileName = $audio['name'];
    $fileTmpName = $fiaudiole['tmp_name'];
    $fileSize = $audio['size'];
    $fileError = $audio['error'];

   
	$update_query = " UPDATE `audio` SET `song_name`=' $song_name',`audio`='$audio',`singer`=' $singer' WHERE id = $id";
    if ($insert_query) {
        // move_uploaded_file($lyrics, $audio_folder);
        echo "
        <script>
        alert('Update Successfully');
            window.location.href = 'audioview.php'
        </script>
        ";
    } else {
        echo "Update FAILED...!";
...
?>
<?php
$uploadsong = 'audio/'; // Specify the directory where you want to save the uploaded songs
...
if (isset($_POST['add_music'])) {
    ...
    if (in_array($fileExtension, $allowedExtensions)) {
        if ($fileError === 0) {
            ...
            $destination = $uploadsong . $newFileName;
...
<head>
		...
		<title>Stream Vibes audioupdate</title>
			...
			<div class="content-wrapper">
				...
				<section class="content">
					...
					<div class=" container ">
                            ...
                            <?php
                            include('connect.php');
                            $select_product = mysqli_query($connection, "SELECT * FROM `audio`");
                            if (mysqli_num_rows($select_product) > 0) {
                                while ($row = mysqli_fetch_assoc($select_product)) {
                            ?>
                                  
                                  <form action="audio.php" enctype="multipart/form-data" method="post">
						<label for="">Song Name</label>
						<input type="text" class="form-control" value="<?php echo $row['song_name']?>"  name="song_name" id="">
						<label for="">Audio</label>
						<input type="file" class="form-control" value="<?php echo $row['audio']?>"  name="audio" id="" accept=".mp3">
						<label for="">Singer</label>
						<input type="text" class="form-control" value="<?php echo $row['singer']?>" name="singer" id="">
						<br>
						<input type="submit" class="btn" style="border:1px solid #ED441D;color:#ED441D;"  value="Update Music" name="update_music" id="">
					</form>
                            <?php
                                }
                            }
